def encode_text(text: str) -> str | None:
    """Кодирует текст в числовое представление."""
    encoding_map = {
        " ": "0",
        ".": "1",
        ",": "11",
        "?": "111",
        "!": "1111",
        ":": "11111",
        ";": "111111",
        "а": "2", "б": "22", "в": "222", "г": "2222",
        "д": "3", "е": "33", "ж": "333", "з": "3333",
        "и": "4", "й": "44", "к": "444", "л": "4444",
        "м": "5", "н": "55", "о": "555", "п": "5555",
        "р": "6", "с": "66", "т": "666", "у": "6666",
        "ф": "7", "х": "77", "ц": "777", "ч": "7777",
        "ш": "8", "щ": "88", "ъ": "888", "ы": "8888",
        "ь": "9", "э": "99", "ю": "999", "я": "9999"
    }

    result = []

    for char in text:
        if is_correct_2(char):
            result.append(encoding_map[char])
        else:
            return None

    # Присоединяем кодированные символы и добавляем пробелы между ними
    encoded_text = " ".join(result)

    return encoded_text if encoded_text else None


def is_correct_2(l: str) -> bool:
    valid_chars = set(" .,!?:;абвгдежзийклмнопрстууфхцчшщъыьэюя")
    return l in valid_chars
